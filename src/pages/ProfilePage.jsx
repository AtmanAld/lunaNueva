...
import { Settings, Pencil, Star, Wallet, Moon, BookOpen, Check, Undo2, Image, CloudUpload, Droplet, Cookie, Bath, Gamepad2 } from 'lucide-react';
        ...
        actions: (
          ...
          <div className="flex flex-col w-full gap-5 px-8 max-w-[340px] mx-auto mt-6 pb-4">
            ...
            <div className="w-full flex justify-center mb-2">
              <GlassCard className="w-full aspect-[4/3] border-2 border-dashed border-primary-fixed/20 bg-surface-container/30 flex flex-col items-center justify-center gap-3 rounded-[2rem] p-6 text-center shadow-[0_0_30px_rgba(0,0,0,0.1)_inset] transition-colors hover:bg-surface-container/50">
                ...
                <Button variant="none" className="bg-surface-container border border-white/5 py-2.5 px-6 rounded-full flex items-center justify-center gap-2 text-on-surface font-medium text-[13px] hover:bg-surface-container-high transition-colors mt-2 shadow-sm">
                  <Image size={16} className="opacity-80" /> <span className="mt-[2px]">Subir archivo</span>
                </Button>
              </GlassCard>
            </div>
            ...
            <div className="flex flex-col gap-2 w-full">
              ...
              <Button onClick={() => setModalType('edit_config')} variant="modal_normal">
                <Settings size={18} /> Editar otras cosas
              </Button>
              <Button onClick={() => setModalType('idle')} variant="modal_normal">
          ...
          <div className="flex flex-col w-full gap-5 px-8 max-w-[340px] mx-auto mt-6 pb-4">
            ...
            <div className="w-full flex flex-col gap-4 mb-2 mt-4">
              <div className="flex flex-col gap-2">
                <label className="text-on-surface-variant text-[9px] uppercase tracking-[0.2em] font-bold pl-4">Nombre de Usuario</label>
                <input
                  type="text"
              ...
              <div className="flex flex-col gap-2 mt-2">
                <label className="text-on-surface-variant text-[9px] uppercase tracking-[0.2em] font-bold pl-4">Nombre de Mascota</label>
                <input
                  type="text"
                  placeholder="Spiral"
                  className="w-full bg-surface-container-low border border-white/5 rounded-[1.5rem] px-5 py-4 text-on-surface font-medium text-[15px] focus:outline-none focus:border-primary-fixed/30 focus:bg-surface-container transition-all placeholder:text-on-surface-variant/30 shadow-[0_4px_12px_rgba(0,0,0,0.1)_inset]"
  ...
  return (
    <>
      <PageDepthWrapper isActive={modalType !== 'idle'} className="p-6 pt-8 pb-32 flex flex-col items-center">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="relative flex flex-col items-center mb-10 w-full mt-2">
          ...
          <div className="relative z-10 mb-6">
            ...
            <div className="w-48 h-48 rounded-full border-[5px] border-secondary shadow-[0_0_30px_rgba(236,185,200,0.2)] flex items-center justify-center bg-surface-container-highest overflow-hidden relative">
              {/* Placeholder Fotográfico de la Usuaria */}
          ...
          <div className="text-center relative z-20 flex flex-col items-center">
            ...
            <p className="text-[11px] font-medium text-on-surface-variant uppercase tracking-[0.25em]">
              Nivel: {userLevel}
            </p>
          </div>
        </motion.div>
        ...
        {/* 3. Zona de Pruebas (Debug) */}
        <div className="w-full flex flex-col items-center gap-4 max-w-[340px] mt-4 opacity-50 hover:opacity-100 transition-opacity">
          <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-on-surface-variant/40">Zona de Pruebas</p>
          ...
          <div className="mt-8 pt-6 border-t border-error/20 flex flex-col gap-3">
            <p className="text-[10px] text-error/60 uppercase tracking-[0.2em] font-bold text-center mb-2">Zona de Peligro (Dev Only)</p>
            <Button 
              variant="none" 
              onClick={() => {
                if(confirm("¿Estás segura? Esto borrará TODO el progreso y reiniciará la app para sincronizar el código.")) {
                  localStorage.removeItem('luna-nueva-storage');
                  window.location.reload();
                }
              }}
              className="bg-error/10 border border-error/30 py-3 rounded-[1rem] text-[11px] text-error uppercase tracking-widest font-bold hover:bg-error/20 transition-all shadow-[0_0_20px_rgba(255,0,0,0.1)]"
            >
              Borrar Datos y Reiniciar App
            </Button>
          </div>
          ...
          <div className="mt-6 flex flex-col gap-2">
            <Button 
              ...
              className="bg-primary/20 border border-primary/50 py-3 rounded-[1rem] text-[11px] text-primary-fixed uppercase tracking-widest font-bold hover:bg-primary/30 transition-all shadow-[0_0_15px_rgba(220,184,255,0.15)]"
            >
          ...
          <div className="mt-8 pt-6 border-t border-primary/20 flex flex-col gap-3">
            ...
            <Button 
              ...
              className="mt-2 bg-secondary/10 border border-secondary/30 py-3 rounded-[1rem] text-[11px] text-secondary uppercase tracking-widest font-bold hover:bg-secondary/20 transition-all"
            >
            ...
            <Button 
              ...
              className="mt-2 bg-error/10 border border-error/30 py-3 rounded-[1rem] text-[11px] text-error uppercase tracking-widest font-bold hover:bg-error/20 transition-all"
            >
